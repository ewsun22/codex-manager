...
5. 编辑 `AGENTS.md` 前先在设置中授权项目根目录；使用供应商网关前先保存供应商，再明确启动 loopback listener。
...
- **官方 App Server adapter：** 以短生命周期读取账户、套餐和 ChatGPT Codex 额度，不附着 Codex Desktop 私有 stdio，也不读取历史数据。
...
官方订阅继续使用可信的 `codex login`、App Server、官方 credential store 和明确账户切换；OAuth access/refresh token 不会进入网关。可选网关默认停止，启动后只绑定 `127.0.0.1`，支持 `/v1/models`、`/v1/responses` 和 `/v1/responses/compact`，不转换其他协议。远程上游必须是 HTTPS 公网 origin；拒绝 redirect、私网/link-local、userinfo、query 和 fragment。客户端 bearer 不转发，请求/响应 body、header、query 和完整上游错误不持久化。配置片段需原生确认，应用不会自动编辑 `config.toml`；停止前请恢复直连配置。
