...
export function CodexGatewayQuickControl({ onOpenGateway }: { onOpenGateway: () => void }) {
  ...
  const stopGateway = async () => {
    ...
    try {
      ...
      setMessage(t("CLIProxyAPI 反代已关闭。若 Codex 当前路由仍指向本地代理，请到“Codex 配置”恢复直连。"));
    } catch (error) {
  ...
  return <section className={`panel gateway-quick-panel${running ? " is-running" : ""}`} aria-labelledby="gateway-quick-heading" aria-busy={busy !== null}>
    ...
    {status?.lastError ? <p className="gateway-quick-error" role="alert">{tt`最近错误：${status.lastError}`}</p> : null}<p className="gateway-quick-message" aria-live="polite">{message}</p>
  </section>;
}
...
export function CodexGatewayView({ onNotice }: { onNotice: GatewayNotice }) {
  ...
  const importOauthProfile = async () => {
    ...
    try {
      ...
      await refresh();
      onNotice("success", outcome.action === "updated" ? tt`OAuth 档案“${outcome.profile.label}”已更新。` : tt`OAuth 档案“${outcome.profile.label}”已导入。`);
    } catch (error) {
  ...
  const setOauthEnabled = async (profile: ProxyAuthProfile, enabled: boolean) => {
    ...
    try {
      ...
      await refresh();
      onNotice("success", tt`OAuth 档案“${profile.label}”已${enabled ? t("启用") : t("禁用")}。`);
  ...
  const deleteOauthProfile = async (profile: ProxyAuthProfile) => {
    ...
    try {
      ...
      await refresh();
      onNotice("success", tt`OAuth 档案“${profile.label}”已移除。`);
  ...
  const restoreOauthProfile = async (profile: ProxyAuthProfile) => {
    ...
    try {
      ...
      await refresh();
      onNotice("success", tt`OAuth 档案“${profile.label}”已恢复，需手动启用后才会进入凭据池。`);
