...
export function CodexGatewayView({ onNotice }: { onNotice: GatewayNotice }) {
  ...
  const { status, setStatus, refreshStatus, statusError, checkedAt } = useGatewayStatus(blocked);
  ...
  const setOauthEnabled = (profile: ProxyAuthProfile, enabled: boolean) => runAction(async () => {
  ...
  const deleteOauthProfile = (profile: ProxyAuthProfile) => runAction(async () => {
  ...
  }, { title: t("移除 OAuth 档案？"), message: tt`移除 OAuth 档案“${profile.label}”？不会删除官方 Codex OAuth。`, confirmLabel: t("移除"), destructive: true });
  const restoreOauthProfile = (profile: ProxyAuthProfile) => runAction(async () => {
  ...
  const updatePort = () => runAction(async () => {
    setBusy("port");
    try {
  ...
  const checkCore = () => runAction(async () => {
    setBusy("core-check");
    try {
  ...
  const installCore = () => runAction(async () => {
  ...
  const startGateway = () => runAction(async () => {
  ...
  const stopGateway = () => runAction(async () => {
    setBusy("stop");
    try {
      const notice = await commitAndRefresh(() => invokeBackend<CodexGatewayStatus>(COMMANDS.stopCodexGateway), setStatus,
