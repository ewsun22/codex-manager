...
v0.6.7 的限定发布例外：在上述真实 OAuth 未验收与 v0.6.6 启动容量故障已披露后，用户明确授权“提交，推送，发布 0.6.7，更新我自己来”。本补丁只调整 SQLite 容量预算及相关验证，不改变 OAuth、网络、Keychain 或内核边界；保留真实 OAuth 未验收状态，不执行账户调用或本机更新。签名、公证、Gatekeeper、updater 验签和草稿/公开资产复验门禁全部保留。本例外只适用于 v0.6.7。
