...
设置中的保留天数用于清理旧 turn/session 与 OTel 元数据，范围为 1–3650 天。AGENTS revision 使用独立策略：每个文件保留最近 20 版，不随活动保留期删除。认证档案删除使用另一独立策略：不能删除活动档案或最后一个可用档案；其他档案先在 Keychain 内软删除并保留 30 天，读取档案列表时会清理过期项目，保留期内可恢复。
