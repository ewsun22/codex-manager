...
- 用户显式启动本地代理后，应用会从独立 Keychain 读取所选 CLIProxyAPI OAuth 档案或 API Key 上游，投影到随机私有 `0700` auth-dir/`0600` 文件或运行配置，并启动官方预编译 sidecar；发送到本机 loopback 的请求由该 sidecar 处理。
...
- Codex provider 的随机 id、名称、规范化 Base URL、模型、reasoning effort、创建/更新时间，以及用户选择的本机端口。API Key 与 CLIProxyAPI OAuth 秘密分别保存在隔离的应用专用 macOS Keychain item，不进入 SQLite；sidecar 运行期间才进入应用私有 `0700` runtime 的 `0600` 配置/auth 文件，checkpoint 后删除。
...
- 供应商 API Key、CLIProxyAPI OAuth token、反代请求/响应 body、header、query、完整上游错误正文和用户内容不会进入 SQLite、普通 WebView DTO、应用日志或备份；唯一落盘例外是 sidecar 运行所必需的私有 `0600` 配置/auth 文件，停止前 checkpoint 后删除。
...
- CLIProxyAPI 反代默认停止，只有用户选择 OAuth 池或 API-key 上游并点击开启后才启动受管 sidecar；生成配置固定 `127.0.0.1`，关闭远程管理、控制面板、插件、usage statistics 和请求日志，同时启用 `commercial-mode`。应用不开放 CLIProxyAPI Management API 或 OAuth callback，不自动安装 CA、系统代理、服务或 root helper。
...
- 用户显式检查、安装或首次开启反代时，应用会访问 `api.github.com` 和 GitHub Release 资产地址以取得 CLIProxyAPI 版本、`checksums.txt` 与平台预编译包；GitHub 会常规看到网络元数据。应用不会上传 Codex 消息、项目或凭据。没有 Release SHA-256 digest、校验清单不一致、资产超限或解压越界时拒绝安装。
